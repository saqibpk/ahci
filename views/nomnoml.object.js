...
$(function (){
	...
	nomnoml.toggleSidebar = function (id){
		...
		for (var r in response) {
		  ...
		  obj.appendChild(elem);
				...
				else if(response[r].type=='sequence diagram'){
	...
	nomnoml.newObject=function(){
	 ...
	 nomnoml.extract=function(){
		 	$.get("/getSome/:2",function(response,status){
		for(var r in response){
			(function(r){
		var obj=document.getElementById("use_list");
		  var anchor = document.createElement("a");
		  anchor.addEventListener("click", function(){ console.log(response[r]); });
		  anchor.id='anchr';
		  anchor.href='#';
		  anchor.innerText = response[r].name;
		  var elem = document.createElement("li");
